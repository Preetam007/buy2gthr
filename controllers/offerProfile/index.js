...
exports.list = function(req, res)
{
    ...
    //console.log(offer_id);
     ...
     if (!req.xhr)
       {
connection.query("select plan_name,views,DATEDIFF(date_posted,NOW()) as date_diff ,substring(offer_desc,1,70) as offer_desc,"+
 "min_price, coupon_code ,type ,comp_link ,link,company,brand ,ol.user_id as user_id from offers of left join offer_love ol "+
        "on of.id = ol.offer_id  where of.id= ?",[offer_id], function(err,rows)
         {
            if(!err)
            {
                res.render('./offerProfile.dust',
                {
                    ...
                    date_diff : rows[0].date_diff == 0 ? 'posted today':"posted "+Math.abs(rows[0].date_diff)+" days ago" ,
